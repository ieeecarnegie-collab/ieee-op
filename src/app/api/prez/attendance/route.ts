...
async function getPrezNotes(): Promise<MeetingNoteAttendance[]> {
  return db
    .select({
      id: meetingNotes.id,
...
export async function GET() {
  ...
  const notes = await getPrezNotes();
...
export async function POST(req: Request) {
  ...
  const [prez] = await db
    .select()
    .from(committees)
    .where(eq(committees.id, PREZ_COMMITTEE_ID));

  if (!prez) {
    return NextResponse.json({ error: "Prez committee not found." }, { status: 500 });
  }
